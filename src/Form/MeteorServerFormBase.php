...
/**
 * @file
 * Contains Drupal\meteor\Form\MeteorServerFormBase.
 */
 ...
 * but instead through the child classes of MeteorServerAddForm and
 * MeteorServerEditForm.
 *
 * @package Drupal\meteor\Form
 *
 * @ingroup meteor
 */
class MeteorServerFormBase extends EntityForm {
  ...
  /**
   * Construct the MeteorServerFormBase.
   *
   * For simple entity forms, there's no need for a constructor. Our Meteor
   * server form base, however, requires an entity query factory to be injected
   * into it from the container. We later use this query factory to build an
   * entity query for the exists() method.
   *
   * @param \Drupal\Core\Entity\Query\QueryFactory $query_factory
   *   An entity query factory for the Meteor Server entity type.
   */
  ...
  /**
   * Factory method for MeteorServerFormBase.
   *
   ...
   *
   * @return array
   *   An associative array containing the Meteor Server add/edit form.
   */
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    // annotation on our MeteorServer class.
    $server = $this->entity;
    ...
    $form['appKey'] = array(
      ...
      '#default_value' => $server->getAppKey(),
    );
  ...
  /**
   * Checks for an existing Meteor Server .
   *
  ...
  public function exists($entity_id, array $element, FormStateInterface $form_state) {
    // Use the query factory to build a new Meteor Server entity query.
    $query = $this->entityQueryFactory->get('meteor_server');
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status == SAVED_UPDATED) {
      ...
      drupal_set_message($this->t('Meteor Server %label has been updated.', array('%label' => $server->label())));
      $this->logger('contact')->notice('Meteor Server %label has been updated.', ['%label' => $server->label(), 'link' => $edit_link]);
    }
    else {
      // If we created a new entity...
      drupal_set_message($this->t('Meteor Server %label has been added.', array('%label' => $server->label())));
      $this->logger('contact')->notice('Meteor Server %label has been added.', ['%label' => $server->label(), 'link' => $edit_link]);
    }

    // Redirect the user back to the listing route after the save operation.
    $form_state->setRedirect('entity.meteor_server.list');
  }
